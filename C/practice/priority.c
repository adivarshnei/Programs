...
void
insert_at_end(struct Node* list, struct Process process) {
...
struct Process*
Process_init_process(struct Process* this) {
...
void
Process_print_process(struct Process* this) {
...
void
Process_calc_tat_wait(struct Process* this) {
...
void
print_processes(struct Node* list) {
    int i = 0;
...
execute_processes() {
    ...
    while ( executing == TRUE ) {
        ...
        for ( i = 0; i < list_size(job_queue); i++ ) {
            if ( get_element(job_queue, i).arrival - time <= 0 &&
                 get_element(job_queue, i).in_ready_queue == FALSE ) {
