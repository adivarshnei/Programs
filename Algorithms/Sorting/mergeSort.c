...
void
merge(int* array, int bottom, int middle, int top) {
    int        i = bottom, j = middle + 1;
    LoopVar    k;
    static int aux[ARRAY_SIZE];
...
void
sort(int* array, int bottom, int top) {
    int middle;

    if ( top <= bottom || isSorted(array) ) {
        return;
    }
