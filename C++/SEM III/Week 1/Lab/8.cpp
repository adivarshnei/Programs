...
int shouldRepeat;
const IntArr daysPerMonth[12] = { 31,28,31,30,31,30,31,31,30,31,30,31 };
const string monthNames[12][15] = {
   "January",
   "February",
   "March",
   "April",
...
int main(void) {
   ...
   while (1) {
      ...
      std::cout << "Distance between both dates is " << distance << " days" << std::endl;
...
Date dateEntry(void) {
   ...
   do {
      ...
      } else if (inputDate.day < 0 || inputDate.day > daysPerMonth[inputDate.month - 1]) {
         std::cout << "Invalid Day" << std::endl;
      }
   } while (inputDate.day < 0 || inputDate.day > daysPerMonth[inputDate.month - 1]);

   return inputDate;
}

inline void printDate(Date date) {
   std::cout << date.day << " " << monthNames[date.month - 1] << ", " << date.year << std::endl;
}

Bool leapChk(int year) {
   Bool isLeap;
...
int distanceBetweenDates(Date date1, Date date2) {
   long int dist1 = 0;
   long int dist2 = 0;
